...
def createBoard():
    font_obj = pygame.font.Font('freesansbold.ttf', 25)
    text_obj = font_obj.render('Circles Wins: ' + str(circlesWins) + ' Xs Wins: ' + str(xsWins), True, black)
...
def createpiece(pos, piece):
    if pos is not None:
        ...
        if piece % 2 == 0 and pieceexist is False:
            ...
            return piece
        if piece % 2 != 0 and pieceexist is False:
            pygame.draw.line(screen, black, (x, y), (x + 100, y + 100), 2)
            pygame.draw.line(screen, black, (x + 100, y), (x, y + 100), 2)
            xs.append(pos)
            piece += 1
            pieceexist = False
            return piece
